...
export const ScreenSaverView = () => {
  const t = useTranslations("SystemSettings.ScreenSaver");
  ...
  return (
    <div className="space-y-6">
      ...
      <SettingsGroup title={t("ScreenSavers")}>
          ...
          {[
            ...
            <div key={name} className="flex flex-col gap-1">
              ...
              <span className="text-xs text-center dark:text-gray-300">
                {name === "Big Sur"
                  ? t("ScreensaverNames.BigSur")
                  : t(`ScreensaverNames.${name}`)}
              </span>
            </div>
          ))}
        </div>
      ...
      <SettingsGroup>
        <SettingsRow label={t("StartAfter")} value={t("FiveMinutes")} />
        <SettingsRow
          label={t("ShowWithClock")}
          type="toggle"
          value={true}
          isLast
        />
